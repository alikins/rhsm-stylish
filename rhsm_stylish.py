...
#    under the License.
#
# heavily based on hacking.py from https://github.com/openstack/nova

"""rhsm code checker
Built on top of pep8.py/hacking.py/flake8
"""
...
def rhsm_except_format(logical_line):
    ...
    if logical_line.startswith("except:"):
        yield 6, "R201: no 'except:' at least use 'except Exception:'"

rhsm_except_format.name = name
rhsm_except_format.version = version


def rhsm_except_format_assert(logical_line):
    r"""Check for 'assertRaises(Exception'.

    rhsm stylish guide recommends not using assertRaises(Exception...):
    ...
    if logical_line.startswith("self.assertRaises(Exception"):
        yield 1, "R202: assertRaises Exception too broad"

rhsm_except_format_assert.name = name
rhsm_except_format_assert.version = version
...
def rhsm_import_alphabetical(logical_line, blank_lines, previous_logical,
                             indent_level, previous_indent_level):
    r"""Check for imports in alphabetical order.

    rhsm HACKING guide recommendation for imports:
    ...
    if blank_lines < 1 and indent_level == previous_indent_level:
        ...
        if (len(split_line) in length and len(split_previous) in length and
            ...
            if split_line[1] < split_previous[1]:
                yield (0, "R306: imports not in alphabetical order (%s, %s)"
                       % (split_previous[1], split_line[1]))

rhsm_import_alphabetical.name = name
rhsm_import_alphabetical.version = version
...
#def nova_no_fail(physical_line):
#    return (0, "E999: FAIL!!")
    ...
    while True:
        ...
        if (token_type == tokenize.NAME and text == "_" and
            ...
            if token_type != tokenize.OP:
                raise LocalizationError(start,
                    "R701: Invalid localization call")
            if text != ")":
                if text == "%":
                    raise LocalizationError(start,
                        "R702: Formatting operation should be outside"
                        " of localization method call")
                elif text == "+":
                    raise LocalizationError(start,
                        "R702: Use bare string concatenation instead"
                        " of +")
                else:
                    raise LocalizationError(start,
                        "R702: Argument to _ must be just a string")
            ...
            if len(positional_specs) > 1:
                raise LocalizationError(start,
                    "R703: Multiple positional placeholders")

# needs to learn gettext/ngettext/N_
def rhsm_localization_strings(logical_line, tokens):
...
rhsm_localization_strings.name = name
rhsm_localization_strings.version = version
